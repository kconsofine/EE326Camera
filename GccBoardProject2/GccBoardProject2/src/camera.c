...
volatile bool g_ul_vsync_flag = false;
...
static uint8_t find_image_length(void) 
{
	...
	while (i < BUFF_SIZE)
	{
		if (g_p_uc_cap_dest_buf[i] == 255 && g_p_uc_cap_dest_buf[i+1] = 216 && begin == 0) // if the current two bytes are FFD8 and FFD9
		{
			begin = i;
		}
		else if (g_p_uc_cap_dest_buf[i] == 255 && g_p_uc_cap_dest_buf[i+1] = 217 && end == 0)
		{
			end = i;
			break; // break out once I find the first end bytes
		}
	}
	if (end == 0 || begin == 0 || end-begin < 0)
	{
		return 0;
	}
	return end-begin + 1;
...
static uint8_t start_capture(void) // edit to return a length
{
	...
	uint8_t success = find_image_length();
	if (success == 0)
	{
		return 0;
	}
